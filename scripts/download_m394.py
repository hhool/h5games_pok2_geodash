...
OUT_DIR = os.path.join(ROOT, 'assets', 'games', 'img', 'm394x236')
TIMEOUT = 20
...
try:
    with open(DATA, 'r', encoding='utf-8') as f:
        text = f.read()
    # Tolerate trailing commas by removing "," before ] or }
    import re
...
for g in games:
    ...
    for src in candidates:
        ...
        if not src.startswith('http'):
            continue
        # replace size tokens
        new_url = src.replace('m184x112','m394x236').replace('m182x112','m394x236').replace('m184x112.jpg','m394x236.jpg')
